...
- `spec_lock.md` — machine-readable execution contract; the compact `audience` / `objective` / `core_message` communication projection plus the exact values Executor must literally use (HEX colors, font stacks, icon library, image resources, and structure mappings)
