...
| Final confirmation evidence exists; `design_spec.md` is missing, with or without a surviving `spec_lock.md` | Return to Generate Step 4 and [`strategist.md`](../../references/strategist.md) §6.2; read final evidence once into the fresh context, scaffold and write `design_spec.md` from it plus source analysis, pass Gate 1, then replace the orphan lock's affected anchors from the completed Design Spec and current context. Never reconstruct the Design Spec from an orphan lock. |
| Final confirmation evidence exists; `design_spec.md` exists and `spec_lock.md` missing | Return to Generate Step 4; in this fresh recovery context read final evidence once to audit the existing Design Spec, then run `project_manager.py scaffold-lock` and author the lock from the audited Design Spec plus current context. |
