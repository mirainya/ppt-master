...
1. **Read** — `read_file references/image-layout-patterns.md`. The file uses stable IDs and separates **Part 1 — Primary Structures** from **Part 2 — Modifier Layers**. Read every catalog entry; ID gaps are intentional, and no remembered count or maximum ID limits selection. The four `Image narrative intent` values below cover only broad categories.
