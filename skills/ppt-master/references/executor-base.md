...
- **Containers are structural**: cards and grids express grouping or parallel capacity, not a house style. Use them only when the content relationship requires them; derive radius, fill, stroke, and depth from the active Design Spec and `spec_lock.md`. Chart-catalog adaptation is owned by [`executor-chart.md`](./executor-chart.md), and catalog preview decoration never overrides project styling.
