...
| `spec_lock.md` | Execution contract | Literal colors, typography, icons, images, page rhythm, charts, `template_reuse_scope`, and the route's PowerPoint structure mode; mirror/layout template routes additionally own input prototypes, the Master roster, and the complete page-to-Master/Layout mapping | Strategist writes the route-specific contract; `page-context` projects the current-page subset for Executor. Executor may add a new adaptive Layout identity only on a structured mirror/layout route while authoring the page that first needs it. |
| `project_manager.py page-context` stdout | Derived per-page context | Read-only model-facing projection of current `design_spec.md`, `spec_lock.md`, and selected template assets | Generate immediately before each page with `--bundle`; never edit or persist it as a replacement source of truth. Flat bundles contain context only; layout adds the complete selected prototype; mirror adds that prototype plus the minimal text-slot view. |
| `analysis/page-context/P<NN>.usage.json` | Derived context telemetry | Actual model-facing bundle size by page and component | `page-context --record-usage` deterministically replaces that page's snapshot; `page-context-report` summarizes current snapshots. Use token data to evaluate later context tooling, never as content or an execution contract. |
...
| `templates/template_execution_manifest.json` (`v1`) + `templates/template_execution/*.text-slots.json` (`v2-min`) | Template execution index | Compact prototype roster/source-import summary plus per-prototype `selector`, `role`, `current_text`, `text_segments`, and `tspan_count`; the sidecar's top-level integrity hash is tool-only | Read the roster once; `page-context --bundle` validates and strips the tool hash, then includes the selected minimal projection and complete prototype for mirror, or the complete prototype alone for layout. Missing, malformed, or stale derived sidecars fall back to the complete SVG. Attributes, topology, and hashes remain tool-validated; never author from either JSON artifact alone. |
