...
`custom` is a tail case: at most one candidate uses it, and its visual prose must satisfy the catalog's custom grammar. Record the confirmed deck-wide `image_rendering` in `design_spec.md §III` first, then project it to `spec_lock.md colors`; confirmed custom prose also becomes `image_rendering_behavior` and is carried into every AI brief. A confirmed non-custom id needs no behavior row. Ignore legacy `image_palette` rows.
