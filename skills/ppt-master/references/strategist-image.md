...
Before Stage 2, use proposed sources only for candidate construction. After confirmation, discard candidate-only sources, map the confirmed set through [`strategist.md`](./strategist.md) §h, and honor `image_notes`; this module never adds a source.
...
When proposed sources include `ai`, read every entry in [`image-renderings/_index.md`](./image-renderings/_index.md) before constructing Stage 2. Unless the user or active template already names a rendering, place at least three credible, distinct renderings across the coordinated safe/shifted/bold directions; a genuine compatibility shortfall may return fewer with a reason. Each `image_strategy` carries localized `rendering`, `visual`, and `mood` only. Mood includes a recognizable real-world analogy. Image colors always inherit that direction's deck HEX roles; never add an image palette or alter deck colors to rescue a rendering.

`custom` is a tail case: at most one candidate uses it, and its visual prose must satisfy the catalog's custom grammar. A confirmed choice locks one deck-wide `image_rendering` in `design_spec.md §III` and `spec_lock.md colors`; confirmed custom prose also becomes `image_rendering_behavior` and is carried into every AI brief. A confirmed non-custom id needs no behavior row. Ignore legacy `image_palette` rows.
