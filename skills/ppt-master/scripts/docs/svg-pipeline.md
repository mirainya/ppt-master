...
- Every visible authored `<g>` is a module with one explicit boundary. Ordinary groups and placeholder slots use `data-pptx-bounds`; native/preset groups reuse `data-pptx-frame`, while native chart/table groups reuse their explicit object bounds. The checker aggregates missing boundaries, warns when a module exceeds its parent/canvas, and warns when estimable text exceeds its nearest module. It never auto-wraps or resizes text.
